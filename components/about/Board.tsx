...
const Board = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://cms.chasnz.org/wp-json/wp/v2/board-member?orderby=date&order=asc&per_page=12', {
                    next: { revalidate: 20 }
                });
                ...
                const fetchImageUrls = data.map(async (member: any) => {
                    const imageUrlResponse = await fetch(`https://cms.chasnz.org/wp-json/wp/v2/media/${member.acf.image}`, {
                        next: { revalidate: 20 }  // This also revalidates every hour
                    });
        ...
        <section className='py-20 lg:py-40'>
            ...
            {boardMembers.map((member) => (
                <div key={member.id} className="member-card flex flex-col gap-4">
                    <div className="member-image relative min-h-[330px]">
                        ...
                        <div className='dropdown-toggle transition absolute bottom-4 right-5'>
                            <Drawer>
                                ...
                                <DrawerContent>
                                    <div className="content-wrapper py-10 lg:py-20">
                                    <div className="drawer-content flex flex-col lg:flex-row gap-10">
                                        ...
                                        <div className="drawer-text w-full lg:w-2/4 flex flex-col gap-6">
                                            ...
                                            <div className='bio-content' dangerouslySetInnerHTML={{ __html: member.bio }}></div>
                                        </div>
                                    </div>
                                    </div>
                                </DrawerContent>
