    ...
    const fetchData = async () => {
      ...
      try {
        const [postResponse, eventResponse, videoWebinarResponse, researchResponse] = await Promise.all([
          fetch(`https://cms.chasnz.org/wp-json/wp/v2/posts?search=${query}&_embed&per_page=30&categories_exclude=74,5,12,17,22,32,37,42,47,52,57,63,68,28,72`),
          fetch(`https://cms.chasnz.org/wp-json/wp/v2/event?search=${query}&_embed&per_page=30`),
          fetch(`https://cms.chasnz.org/wp-json/wp/v2/posts?search=${query}&categories=74&_embed&per_page=30`),
          fetch(`https://cms.chasnz.org/wp-json/wp/v2/posts?search=${query}&categories=5,12,17,22,32,37,42,47,52,57,63,68,28,72&_embed&per_page=30`)
        ]);
          ...
          <div className="search-box-container-mobile z-50 flex justify-center items-center fixed left-0 right-0 bottom-0 top-0 w-full">
            <div className="search-container fixed w-full lg:w-2/4 h-screen z-50 flex items-center justify-center">
              <div className="search-box w-full h-screen bg-white rounded-xl">
                ...
                <button
                  className={`py-2 px-4 w-1/3 text-black text-sm ${selectedTab === 'posts' ? 'border-b-2 border-[--primary-colour]' : ''}`}
                  onClick={() => setSelectedTab('posts')}
                >
                  Resources
                </button>
                <button
                  className={`py-2 px-4 w-1/3 text-black text-sm ${selectedTab === 'research' ? 'border-b-2 border-[--primary-colour]' : ''}`}
                  onClick={() => setSelectedTab('research')}
                >
                  Research
                </button>
                <button
                  className={`py-2 px-4 w-1/3 text-black text-sm ${selectedTab === 'events' ? 'border-b-2 border-[--primary-colour]' : ''}`}
