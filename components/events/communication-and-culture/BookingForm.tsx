...
const BookingForm: React.FC<BookingFormProps> = ({ onClose }) => {
  ...
  const eventDate = '7th March, 2024';
  ...
  const generateCalendarLinks = (dateString: string) => {
    ...
    const endDate = new Date(parseInt(year), monthNumber - 1, dayNumber, 16, 0);

    const formatDate = (date: Date) => {
      return date.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
    };

    const eventTitle = `Communication and Culture Masterclass`;
    const eventDescription = `Communication and Culture Masterclass Event`;
...
END:VCALENDAR`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/communication-culture', {
        method: 'POST',
        headers: {
    ...
    <div className="workshop-booking-form fixed w-full top-0 right-0 bottom-0 left-0 bg-black/70 z-[9999] flex justify-center items-end md:items-center overflow-scroll">
      <div className="form-container overflow-scroll w-full md:w-4/5 xl:w-2/4 z-50 bg-white rounded-xl p-4 md:p-8 lg:p-10 mx-auto relative">
        ...
        <div className='object-cover w-full rounded-md overflow-hidden mt-8 lg:mt-0'>
          <Image src="/events/communication-and-culture/header-image.jpg" alt="Header Image" className='w-full' width={800} height={300} />
        </div>

        <div className='flex flex-col gap-4 mt-6'>
          <h6 className='text-lg md:text-2xl font-semibold'>Welcome to the Communication and Culture Masterclass registration</h6>
              ...
              <div className=''>
                ...
                <div className="flex flex-row gap-4 mt-4 justify-between flex-wrap">
                  <div>
                    ...
                    <p className='text-base'>Auckland</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className='text-base'>9:00am - 4:00pm</p>
                  </div>
                  <button 
                    type="submit" 
